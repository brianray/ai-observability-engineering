...
> **Note on the path.** The Python project lives in the `ai-observability-engineering/` subdirectory of the repository, so the `cd` above descends two levels. `pyproject.toml`, `src/`, `chapters/`, and `tests/` are all relative to that directory.
...
The test suite is 641 tests: 304 unit tests covering framework internals and the chapter companion code, and 337 functional tests covering every example and every chapter's claims. Six unit tests skip unless the optional extras (`langgraph`, `ragas`) and `promtool` are present.
...
All paths below are relative to `ai-observability-engineering/`, the project directory inside this repository.
...
- **[`docs/INSTRUCTIONS.md`](ai-observability-engineering/docs/INSTRUCTIONS.md)** setup, first run, troubleshooting, using the framework in your own project
- **[`docs/TESTING.md`](ai-observability-engineering/docs/TESTING.md)** the harness in depth, what each assertion catches, how to test instrumentation you already have
- **[`docs/AUTHORING.md`](ai-observability-engineering/docs/AUTHORING.md)** adding a chapter example, the contract it has to meet
- **[`CONTRIBUTING.md`](ai-observability-engineering/CONTRIBUTING.md)** pull request expectations

---

## License

MIT. See [`LICENSE`](ai-observability-engineering/LICENSE).
